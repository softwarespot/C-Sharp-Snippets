        ...
        public static void Main()
        {
            while (true)
            {
                ...
                */
                ...
                // Creating a random integer array of values between 1 and 49: http://stackoverflow.com/questions/14118844/how-does-enumerable-orderby-use-keyselector and Joni for the Enumerable.Range() and Take() method.
                int[] i = { 0 }, userArray = Array.ConvertAll(Regex.Replace(Regex.Replace(Regex.Replace(Console.ReadLine(), @"(\d+)\D*", @"$1,"), @"^,|,$", ""), @"[^\d,]", @"0").Split(','), int.Parse), computerArray = Enumerable.Range(1, 49).OrderBy(element => Guid.NewGuid()).Take(7).OrderBy(element => element).ToArray();  // computerArray = { 0, 0, 0, 0, 0, 0, 0 }; // Integer arrays to compare later using SequenceEqual().
