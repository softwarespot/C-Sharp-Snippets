...
namespace Tic_Tac_Toe
{
    internal class Program
    {
        private static void Main()
        {
            ...
            const int emptyValue = 0, userValue = 1, computerValue = 2,
                _userChoice = 0, _computerChoice = 10;
            ...
            int[,] computerLogic =
			{
				...
				{2, 4, 5, 0, 0, 0, 0, 3}, // Last column is used as a count for the number of possible squares around the user's choice.
				{1, 3, 4, 5, 6, 0, 0, 5},
				{2, 5, 6, 0, 0, 0, 0, 3},
				{1, 2, 5, 7, 8, 0, 0, 5},
				{1, 2, 3, 4, 6, 7, 9, 7},
            ...
            while (randomSequence.Count != 0 && isWin == null)
            {
                ...
                if (!isComputer)
                {
                    table[_computerChoice] = new Random(Environment.TickCount).Next(0, randomSequence.Count - 1); // Use the system tick count as a random seed.
        ...
        private static bool? IsWin(ref int[] table, int userValue, int computerValue)
        {
            return (table[1] == userValue && table[2] == userValue && table[3] == userValue || table[4] == userValue && table[5] == userValue && table[6] == userValue || table[7] == userValue && table[8] == userValue && table[9] == userValue || table[1] == userValue && table[4] == userValue && table[7] == userValue || table[2] == userValue && table[5] == userValue && table[8] == userValue || table[3] == userValue && table[6] == userValue && table[9] == userValue || table[1] == userValue && table[5] == userValue && table[9] == userValue || table[3] == userValue && table[5] == userValue && table[7] == userValue) ? true : ((table[1] == computerValue && table[2] == computerValue && table[3] == computerValue || table[4] == computerValue && table[5] == computerValue && table[6] == computerValue || table[7] == computerValue && table[8] == computerValue && table[9] == computerValue || table[1] == computerValue && table[4] == computerValue && table[7] == computerValue || table[2] == computerValue && table[5] == computerValue && table[8] == computerValue || table[3] == computerValue && table[6] == computerValue && table[9] == computerValue || table[1] == computerValue && table[5] == computerValue && table[9] == computerValue || table[3] == computerValue && table[5] == computerValue && table[7] == computerValue) ? false : (bool?)null);
        }

        private static void Print(ref int[] table)
        {
            Console.Clear();
            for (int i = 1; i <= 9; i++)
                Console.Write(((i == 1) ? "Tic-Tac-Toe by SoftwareSpot (c) 2014\n|-----|-----|-----|\n" : "") + ((table[i] == 1) ? "|--X--" : (table[i] == 2) ? "|--0--" : "|--" + i + "--") + ((i % 3 == 0) ? "|\n|-----|-----|-----|\n" : ""));
        }
    }
}